...
class GameScene: SKScene {
    ...
    private func startGame() {
        self.removeAllChildren()  // Clear all nodes before starting the game
        setupUI()  // Recreate the UI elements
        self.isPaused = false
        ...
        // Start changing fruit names every 5 seconds
    ...
    private func highlightCorrectFruitName() {
        let highlight = SKAction.sequence([
            SKAction.run { self.currentFruitNameLabel?.fontColor = .red },
            SKAction.scale(to: 1.6, duration: 0.2),
    ...
    private func endGame() {
        ...
        self.playAgainButton = SKSpriteNode(color: .blue, size: CGSize(width: 150, height: 50))
    ...
    private func restartGame() {
        // Remove play again button and any game over labels/messages
        self.playAgainButton?.removeFromParent()
        self.enumerateChildNodes(withName: "*") { node, _ in
            if node.name == "gameOverLabel" || node.name == "gameOverMessage" || node.name == "rewardLabel" {
                node.removeFromParent()
            }
        }
